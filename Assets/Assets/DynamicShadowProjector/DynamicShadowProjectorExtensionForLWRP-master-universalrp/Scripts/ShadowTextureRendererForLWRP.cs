...
namespace DynamicShadowProjector
{
	public partial class ShadowTextureRenderer
	{
		...
		private RTHandle m_tempRTHandle;
		private RTHandle m_shadowRTHandle;
		...
		{
			if (useIntermediateTexture && m_temporaryRenderTarget != null)
			{
				if (m_tempRTHandle == null || m_tempRTHandle.rt != m_temporaryRenderTarget)
				{
					m_tempRTHandle?.Release();
					m_tempRTHandle = RTHandles.Alloc(m_temporaryRenderTarget);
				}
			}
			if (shadowTexture != null)
			{
				if (m_shadowRTHandle == null || m_shadowRTHandle.rt != shadowTexture)
				{
					m_shadowRTHandle?.Release();
					m_shadowRTHandle = RTHandles.Alloc(shadowTexture);
				}
			}
		}
		...
		internal void ConfigureRenderTarget(RenderShadowTexturePass pass)
		{
			...
			{
				pass.ConfigureTarget(m_tempRTHandle);
			}
			else
			{
				pass.ConfigureTarget(m_shadowRTHandle);
			}
