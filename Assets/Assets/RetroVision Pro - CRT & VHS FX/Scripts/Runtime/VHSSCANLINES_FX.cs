...
{
    public class VHSSCANLINES_FX : ScriptableRendererFeature
    {
        [SerializeField]
        private Shader m_Shader;
        private Material material;
        VHSSCANLINES_Pass RetroVision_RenderPass;
        public RenderPassEvent Event = RenderPassEvent.BeforeRenderingPostProcessing;

        public override void Create()
        {
            m_Shader = Shader.Find("RetroVisionPro/CRT&VHSFX/VHSScanlines");
            if (m_Shader == null)
            {
                return;
            }
            material = new Material(m_Shader);
            RetroVision_RenderPass = new VHSSCANLINES_Pass(material);
        ...
        public class VHSSCANLINES_Pass : ScriptableRenderPass
        {
            ...
            public VHSSCANLINES_Pass(Material material)
            {
            ...
            public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
            {
                ...
                material.SetFloat("Time", TimeX);
                int pass;
                material.SetFloat("_ScanlineDensity", volumeComponent.ScanlineDensity.value);
                material.SetFloat("_BandScrollSpeed", volumeComponent.Speed.value);
                material.SetFloat("_JitterDivisor", volumeComponent.JitterDivisor.value);
                material.SetFloat("_BlendAmount", volumeComponent.Fade.value);
                material.SetFloat("_FisheyeSpherical", volumeComponent.FisheyeSpherical.value);
                material.SetFloat("_FisheyeBarrel", volumeComponent.FisheyeBarrel.value);
                material.SetFloat("_FisheyeScale", volumeComponent.FisheyeScale.value);
                material.SetColor("_ScanlineColor", volumeComponent.ScanLinesColor.value);
                if (volumeComponent.EnableHorizontal.value)
                {
                    if (volumeComponent.JitterDivisor.value != 0)
                        pass = 1;
                    else
                        pass = 0;
                }
