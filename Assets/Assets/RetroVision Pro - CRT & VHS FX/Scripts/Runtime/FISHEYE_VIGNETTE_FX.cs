...
{
    public class FISHEYE_VIGNETTE_FX : ScriptableRendererFeature
    {
        [SerializeField]
        private Shader m_Shader;
        private Material material;
        FISHEYE_Pass RetroVision_RenderPass;
        public RenderPassEvent Event = RenderPassEvent.BeforeRenderingPostProcessing;

        public override void Create()
        {
            m_Shader = Shader.Find("RetroVisionPro/CRT&VHSFX/FISHEYE_VIGNETTE");
            if (m_Shader == null)
            {
                return;
            }
            material = new Material(m_Shader);
            RetroVision_RenderPass = new FISHEYE_Pass(material);
        ...
        {
            FISHEYE_VIGNETTE myVolume = VolumeManager.instance.stack?.GetComponent<FISHEYE_VIGNETTE>();
        ...
        public class FISHEYE_Pass : ScriptableRenderPass
        {
            private Material material;

            public FISHEYE_Pass(Material material)
            {
            ...
            public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
            {
                ...
                var volumeComponent = VolumeManager.instance.stack.GetComponent<FISHEYE_VIGNETTE>();

                material.SetFloat("_FisheyeSize", volumeComponent.FisheyeSize.value);
                material.SetFloat("_CutoffX", volumeComponent.CutoffX.value);
                material.SetFloat("_CutoffY", volumeComponent.CutoffY.value);
                material.SetFloat("_CutoffFadeX", volumeComponent.CutoffFadeX.value);
                material.SetFloat("_CutoffFadeY", volumeComponent.CutoffFadeY.value);
                material.SetFloat("_ChromaFringe", volumeComponent.ChromaFringe.value);
